            ...
            if (angles[i] < pi / 2 + pi / 50) and (angles[i] > pi / 2 - pi / 50):
                if self.parking_identified == 0:            # No lot identified
                    ...
                    if self.parking_lot_dist > pp_len_threshold:
                        ...
                        self.pp_corner = [(sin(self.current_heading) * self.pp_range) + self.car_pose.pose.pose.position.x, -(cos(self.current_heading) * self.pp_range)+self.car_pose.pose.pose.position.y] #Corner position
